...
Const $cVersion = "ZakupkiGovRuIngos20160318"
Const $cProduct = "zakupki.gov.ru"
...
Func _MainShowTender()
    ...
    If $iRval = $SQLITE_OK Then
		...
		If $hFileOpen = -1 Then
			...
			;Return False
		EndIf
		FileWrite($hFileOpen, "База найденых тендеров на zakupki.gov.ru от " & @MDAY & '.' & @MON & '.' & @YEAR & "." & @CRLF & @CRLF)
...
Func _MainProcess()
	...
	Local $iFileExists = FileExists(@ScriptDir & "\zakupki.gov.ru.ext.au3")
	If $iFileExists Then
		If $DEBUGMODE Then _FileWriteLog($hLogFile, "_MainProcess: Execute AU3")
		ShellExecute(@ScriptDir & "\zakupki.gov.ru.ext.au3")
	Else
		If $DEBUGMODE Then _FileWriteLog($hLogFile, "_MainProcess: Execute EXE")
		ShellExecute(@ScriptDir & "\zakupki.gov.ru.ext.exe")
	EndIf
